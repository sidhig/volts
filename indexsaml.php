...
if(tbl_role_user.view = 2,'0','1')  as can_edit
 FROM `tbl_role_user` left join tbl_role on tbl_role_user.role = tbl_role.role_name where ntid ='".$_SESSION['samlNameId']."'";
...
<div class="container" style="width:55vw;border: 2px solid #000000;background-color:white;">
 ...
 <table >
      ...
      <tr>
       <td><center><a href ='https://volts.systems/s/demo1/'><input type="hidden" class="btn btn-primary" id="login" value="Log In With SAML" onclick="validateForm()"></a><br/></td>
       </tr>
      <tr>
        <td><center><a href ='https://volts.systems/s/demo1/'><input type="button" class="btn btn-primary" id="login" value="Log In" onclick="validateForm()" style="float:left;width:200px;height:46px;font-size:1vw;"></a><br/></td>
      </tr>
  ...
  <div id="error" style=" <?  if($err!='1'){ echo "display:none;"; }?> width:86%; height:60px; top:5px; color:red;"> 
    <h5 style="width: 107%;">Sorry! You are not an authorized VOLTS user</h5>
  </div>  
</div>    
</form>
</div>
