...
<title>Get Braking Report</title>
...
<tr>
<th align="left" style="width: 462px;"><input onclick='window.close();' type="button" value="Back" style="float:left;margin-top:2vh;" /></th></tr>
</table>
<center><h1>Braking Report</h1></center>
<form method="post" action="" style="border:solid 1px black; margin-top: 20px;width:93%; padding:20px;" id="get_usage_report" onsubmit="return validateForm()" >
<table border="0" width="96%" style=" border-spacing: 15px;">
...
<?php
		 	$result = $conn->query("select DeviceName,DeviceIMEI from devicedetails where username='gpc' and OBDType='OBDII' and opco in (select name from role_opco where if('".$_SESSION['ROLE_opco']."'='0',1,id in (".$_SESSION['ROLE_opco']."))) and 
...
DeviceType in (SELECT eq_name from tbl_eqptype where if('".$_SESSION['ROLE_eqp']."'='0',1,id in (".$_SESSION['ROLE_eqp']."))) order by trim(DeviceName)");
...
<!--<center><strong>Deacceleration:</strong> <input type="number" name="decel" value="10" />
...
$("#get_usage_report").attr("action","braking_report.php");
