...
<center><h1>Usage Report</h1></center>
...
<?php
		 	$result = $conn->query("select DeviceType from devicedetails where OBDType = 'Wired' and UserName = 'gpc' and CASE WHEN (select dept  as eqpval  from roletable where username = '".$_SESSION['LOGIN_user']."') = 'All' THEN 1 ELSE (select dept from roletable where username = '".$_SESSION['LOGIN_user']."') like CONCAT('%', department, '%')  END and  CASE WHEN (select `group`  as eqpval  from roletable where username = '".$_SESSION['LOGIN_user']."') = 'All' THEN 1 ELSE (select `group` from roletable where username = '".$_SESSION['LOGIN_user']."') like CONCAT('%', `group`, '%')  END group by DeviceType");
...
<?php
		 	$result = $conn->query("select DeviceName,DeviceIMEI from devicedetails where username = 'gpc' and CASE WHEN (select dept  as eqpval  from roletable where username = '".$_SESSION['LOGIN_user']."') = 'All' THEN 1 ELSE (select dept from roletable where username = '".$_SESSION['LOGIN_user']."') like CONCAT('%', department, '%')  END and  CASE WHEN (select `group`  as eqpval  from roletable where username = '".$_SESSION['LOGIN_user']."') = 'All' THEN 1 ELSE (select `group` from roletable where username = '".$_SESSION['LOGIN_user']."') like CONCAT('%', `group`, '%')  END group by DeviceIMEI");
			 while($row = $result->fetch_object())
			 {
...
-->
<input type="button" id="get_dt_rep" value="Get Report" /> <input type="button" id="get_graph" value="Get Graphic Report" /><input type="hidden" id="get_ov_rep" value="Overview Report" /><br /><br />
...
var equip_type = $("#equip_type").val();
		$.post( "get_eqp_wired.php",{ equip_type: equip_type },function(data) {
		$("#veh_name").empty();
...
$("#get_usage_report").attr("action","usage_report.php");
$("#get_usage_report").submit();
});
$("#get_graph").click( function() {
$("#get_usage_report").attr("action","usage_graph_report.php");
$("#get_usage_report").submit();
});
$("#get_ov_rep").click( function() {
$("#get_usage_report").attr("action","usage_ov_report.php");
$("#get_usage_report").submit();
});
