...
if ($result = $conn->query("select * from tbl_role_user where id = '".$_REQUEST['id']."'")) { 
        $obj = $result->fetch_object();
  }
  $obj_main = $conn->query("SELECT  * FROM `tbl_role` where role_name='".$_POST['role']."' ")->fetch_object();
?>
...
<h1 style="font-size:2.2rem;width: 50vw;"><b>User Details</b></h1><h1 style="margin-top: -4.5vh;float:right;font-size:2.2rem;width: 60vw;"><b>Preferences</b></h1>
...
<!--   <input type="button"  id="edit_role" value="Edit Role" style="height:25px;margin-top:19px;"> -->
   ...
   <table style="font-size:small;">
					  <tr>
                            ...
                            <td><input id="name" name="user_name" type="text" class="intp" placeholder="Name"  value='<?=($obj->name)?>'>
                            </td>
                      </tr>
					  <tr>
                            <td><strong class="stg">NTID:</strong></td>
                            <td><input id="ntid" name="ntid" type="text" class="intp" placeholder="NTID"  value='<?=($obj->ntid)?>'>
                            </td>
                      </tr>
                      <tr>
                            <td><strong class="stg">Email:</strong></td>
                            <td><input id="email" name="user_email" type="text" class="intp" placeholder="Email"  value='<?=($obj->emailid)?>'>
                            </td>
                      </tr>
                      <tr>
                          <td><strong class="stg">Username:</strong></td>
                          <td><input type='hidden' name='qry_type' value='edit'>
                             <input name="edit_id" type="hidden" value='<?=($obj->id)?>'>
                             <input name="edit_id1" type="hidden" value='<?=($obj_main->role_name)?>'>
                            <input id="username" name="user_username" type="text" class="intp" style="background-color:#D3D3D3;" placeholder="Username" value='<?=($obj->username)?>' readonly>
                          </td>
                      </tr>
                      <tr>
                            <td><strong class="stg">Password:</strong></td>
                            <td><input id="password" name="user_password" type="text" class="intp" placeholder="Password"  value='<?=($obj->password)?>'>
                            </td>
                      </tr>
                      ...
                      <tr>      
                          ...
                          <td>
                              <select id="view" name="view_role" class="intp" style="background-color:#D3D3D3;">  
                                <?if($_SESSION['LOGIN_role'] =='admin'){?>
                                    <option value='-1' disabled="disabled" selected>Select</option>
                                    <option value='2' <? if($obj->view=='2'){ ?> selected <? } ?>>View olny</option>
                                    <option value='3' <? if($obj->view=='3'){ ?> selected <? } ?>>Normal</option>
  ...
  <tr>
      ...
      <select  id="opco_role" multiple="multiple" size="1" >
                  <? $opco_role_id=explode(',',$obj_main->opco); 
                      $sql = $conn->query("SELECT * from role_opco where if('".$_SESSION['ROLE_opco']."'='0',1,id in (".$_SESSION['ROLE_opco'].")) order by name asc");
                      while($obj = $sql->fetch_object()){
                        
                  ?>
                      <option value="<?=$obj->id?>" <? foreach($opco_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
      ...
      <select id="primary_role" multiple="multiple" size="1" >
                  <?  $primary_role_id=explode(',',$obj_main->primary); 
                      $sql = $conn->query("SELECT * from role_primary where if('".$_SESSION['ROLE_primary']."'='0',1,id in (".$_SESSION['ROLE_primary'].")) order by name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option  value="<?=$obj->id?>" <? foreach($primary_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
  ...
  <tr>
      ...
      <select  id="group_role" multiple="multiple" size="1" >
                  <?  $group_role_id=explode(',',$obj_main->group); 
                      $sql = $conn->query("SELECT * from role_group where if('".$_SESSION['ROLE_group']."'='0',1,id in (".$_SESSION['ROLE_group'].")) order by name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option value="<?=$obj->id?>" <? foreach($group_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
      ...
      <select  id="dept_role" multiple="multiple" size="1" >
                  <?  $dept_role_id=explode(',',$obj_main->department); 
                      $sql = $conn->query("SELECT * from role_dept where if('".$_SESSION['ROLE_dept']."'='0',1,id in (".$_SESSION['ROLE_dept'].")) order by name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option value="<?=$obj->id?>" <? foreach($dept_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
 ...
 <tr>
    <td><b>Workgroup</b></td>
      ...
      <select  id="work_group" multiple="multiple" size="1" >
                  <?  $work_role_id=explode(',',$obj_main->workgroup);
                      ...
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option   value="<?=$obj->id?>" <? foreach($work_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
      ...
      <select  id="eqp_role" multiple="multiple" size="1" >
                  <?  $eqp_role_id=explode(',',$obj_main->equipment);
                      $sql = $conn->query("SELECT * from tbl_eqptype where if('".$_SESSION['ROLE_eqp']."'='0',1,id in (".$_SESSION['ROLE_eqp'].")) order by eq_name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option <? foreach($eqp_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->eq_name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
  ...
  <tr>
    <td><b>View</b></td>
      ...
      <select id="view_role"  multiple="multiple" size="1">
                  <?  $view_role_id=explode(',',$obj_main->view);
                      $sql = $conn->query("(SELECT * from role_view where if('".$_SESSION['ROLE_view']."'='0',1,id in (".$_SESSION['ROLE_view'].")) )order by name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option  <?if(trim($obj->name) == "Dispatch" || trim($obj->name) == "Activity Search"){ echo "disabled";}?> <? foreach($view_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
      ...
      <select id="report_role"  multiple="multiple" size="1">
                  <?  $report_role_id=explode(',',$obj_main->report);
                      $sql = $conn->query("SELECT * from role_report where if('".$_SESSION['ROLE_view']."'='0',1,id in (".$_SESSION['ROLE_view'].")) order by name asc");
                      while($obj = $sql->fetch_object()){
                  ?>
                      <option <? foreach($report_role_id as $val){ if($val == $obj->id){ echo "selected";}}?> ><?=$obj->name?></option>
                  <?   } 
                  ?>
                </select>
      </td>
  ...
  <input onclick='validateuser();' type='button'  value="Update" class="btn btn-primary" style="height:5vh;width:20vw;margin-top: 3vh;">
</center>

<!--      <div class="col-sm-12" style="width:100%;margin-top:1vh;margin-bottom: 18vh;">
      ...
      <tr><th>Section</th></tr>
         ...
         if ($result = $conn->query($sql)) { 
              ...
              <script>
               ...
               <? foreach($primary_role_id as $val){
              if($val == $obj->id){
              ?>
              <tr style="text-align:left;">
