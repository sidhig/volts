  ...
  //print_r($_POST);
  if($_POST['time_sheet']!=''){
  $obj = $conn->query("select * from relocation where time_sheet = '".$_POST['time_sheet']."'")->fetch_object();
   //print_r($obj);
  }
  ?>
  <style>
...
<tr>
	...
	<th style='text-align:center;width: 40%;'><h3><b>VEHICLE RELOCATION</b></h3></th>
	<td align="right" style='text-align:right;width: 30%;'>Relocation Sheet Number: <strong><?=$obj->time_sheet?></strong></td>
<? if($obj->status!='Cancel' && $obj->status!='Completed' ) { ?>
<tr>
	<td><input type="button" id="cancel_ts" value="Cancel This Relocation Sheet" onclick="rl_change_status('<?=$obj->time_sheet?>','Cancel');" /></th>
	<td></td>
	<td align="right" style='text-align:right;width: 30%;'><input type="button" id="cancel_ts" value="Complete This Relocation Sheet" onclick="rl_change_status('<?=$obj->time_sheet?>','Completed');" /></td>
</tr>
<? } ?>
</table>
</tr>
...
<input type="hidden" name="trip_form" value="edit"  />
  ...
  <div class="col-sm-3" >
    ...
    <span style='margin-top: 4px; float: right; margin-right: 3vw;'><input name="auto_complete" <? if($obj->auto_complete){ echo 'checked'; } ?> type='checkbox' style='margin-bottom: 0px; float: left;' value='1'>&nbsp;Auto Complete </span>
...
$("#driver_phone").val(get_phone($("#contract_driver").val().toLowerCase()));
     ...
     <div class="row">
		...
		<div class="col-sm-3" >
		 <input name="contract_driver" id="contract_driver" list='driver_list' type="text" class="int" style="" value="<?=$obj->contract_driver;?>"/>
		</div>
		<div class="col-sm-1" ></div>
		<div class="col-sm-2" >
     ...
     <div class="row">
		...
		<div class="col-sm-3" >
		  <textarea name="notes" cols="90" class="int" rows='3'><?=$obj->notes;?></textarea>
		</div>
		<div class="col-sm-1" ></div>
		<div class="col-sm-2" >
...
<? include_once('substationjson.php'); ?>
 ...
 <div class="row" id="d_add" <? if($obj->depart_add==''){ ?> style='display:none;' <? } ?> >
  ...
  <? $result = $conn->query("select * from relocation_other_poi where `tsn`='".$obj->time_sheet."'");
      ?>
    ...
    <div class="col-sm-3" >
     <input type="text" name="o_poi<?=$btn?>" list="poi_list" id="o_poi<?=$btn?>" class="int" style="margin-bottom: .5vh;margin-top:.5vh;" value="<?=$other_poi->poi?>" <? if($other_poi->is_in=='1' || $other_poi->is_out=='1'){ ?> disabled <? } ?> />
    </div>
   <div class="col-sm-1" ></div> 
   ...
   <span id="a_add<?=$btn?>" <? if($other_poi->add==''){ ?> style='display:none;' <? } ?>>
	   <div class="col-sm-2"  >
		...
		<div class="col-sm-3" >
		<input type="text" id="o_add<?=$btn?>" name="o_add<?=$btn?>" class="int" value="<?=$other_poi->add?>">
    ...
    ?>" readonly disabled  class="int" style="margin-right: 1.5vw;
    margin-top: .5vw;">
    </div>
    </div>
    </div>
  ...
  <div class="col-sm-3" >
   <input name="depart_time" value="<? if($obj->depart_time!='') { echo date('m/d/Y h:i A',strtotime($obj->arrival_time)); } ?>" disabled class="int">
  </div>
 </div>
...
function rl_change_status(tsn,status) 
{ 
	if(status=='Cancel'){
			var a = prompt('Reason for cancellation:');
				if(a==null){
				}
				else{
					$.ajax({
						...
						data: 'trip_form='+status+'&time_sheet='+tsn+'&reason_cancel='+a,
						success: function (data) {
							alert("Relocation Sheet number "+tsn+" Cancelled successfully.");
						$("#trip_sheet_div").hide();
		...
		if(confirm('Are you want to change the status of this Relocation Sheet ?')){
			 $.ajax({
				type: 'post',
				url: 'relocation_query.php',
				data: 'trip_form='+status+'&time_sheet='+tsn,
				success: function (data) {
...
if (($( "#req_date" ).val().trim() == "") || ($( "#del_date" ).val().trim() == "") || ($( "#contact_person" ).val().trim() == "") || ($( "#contact_no" ).val().trim() == "") || ($( "#req_by" ).val().trim() == "") || ($( "#req_no" ).val().trim() == "") || ($( "#item_name" ).val().trim() == "") || ($( "#status" ).val().trim() == "")) {
      ...
      return false;
        ...
        $("#submit_button").val("Saving Relocation ...");
		var interval = setInterval(function(){   
		  ...
		  {
			count++;
			$("#submit_button").val("Please wait. Retrying to save Relocation ...count " + count);
			setTimeout(function(){
			  form_sub();
			}, 2000);
		  }
...
/**/
  ...
  <center><? if($obj->status!='Cancel' && $obj->status!='Completed' ) { ?>
	 ...
	 <!--  <a target='_blank' href='trip_pdf.php?tsn=<?=$obj->time_sheet?>' ><input type="button" value="Print" /></a> --> 
	  ...
	  <input onclick='$("#trip_sheet_div").hide();$("#trip_sheet_div").html("");' type="button" value="Close" />
  </center>
